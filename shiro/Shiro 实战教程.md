...
##### 2. 引入shiro.ini配置文件并加入如下配置(注：此配置文件用来学习shiro时书写系统中相关权限数据，在springboot整合后无须此配置文件)
...
> MD5算法：
>
> ​	作用：一般用来加密或者签名(校验和，校验两个文件是否一致)
>
> ​	特点：算法不可逆(只能根据明文推算密文，不能通过密文反推明文)、内容相同时无论执行多少次md5生成结果始终是一致的
>
> ​	生成结果：始终是一个16进制32位长度字符串
>
> 加Salt：在明文密码的基础上添加随机字符串，实现加密效果
